...
class DashboardPage(ttk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        ttk.Label(top_right_frame, text=user_display_name, bootstyle="success").pack(side=LEFT, padx=5)

        # Fetch and display the avatar
        response = requests.get(avatar_url)
        avatar_image = Image.open(BytesIO(response.content))

        avatar_photo = ImageTk.PhotoImage(avatar_image.resize((48, 48)))  # Resize the image to 48x48
        ...
        issues = jira.jql("assignee = currentUser() AND resolution = Unresolved")['issues']
        ...
        ttk.Label(ticket_frame, text="Select or Enter Ticket:", bootstyle="info").pack(side=LEFT, padx=5)

        # Combobox for ticket selection with editable entry
        ticket_var = ttk.StringVar()
        ticket_combobox = ttk.Combobox(ticket_frame, textvariable=ticket_var, values=ticket_options)
        ...
        link_icon_image = Image.open("assets/link-icon.png")  # Replace with the actual path to your icon
        link_icon_image = link_icon_image.resize((16, 16))
        link_icon_photo = ImageTk.PhotoImage(link_icon_image)  # Convert to PhotoImage
        link_button = ttk.Button(ticket_frame, image=link_icon_photo, command=open_ticket, bootstyle="link")
        link_button.image = link_icon_photo  # Keep a reference to avoid garbage collection
        ...
        timer_entry = ttk.Entry(ticket_frame, textvariable=timer_var, width=10, justify=CENTER)
        timer_entry.pack(side=LEFT, padx=5)

        # Timer state
        timer_running = [False]  # Use a mutable object to allow modification in nested functions
        ...
        # Reset button
        reset_button = ttk.Button(ticket_frame, text="Reset", command=reset_timer, bootstyle="danger")
        reset_button.pack(side=LEFT, padx=5)

        # Play/Pause button
        play_pause_button = ttk.Button(ticket_frame, text="Play", command=toggle_timer, bootstyle="primary")
        ...
        transfer_button = ttk.Button(self, text="Transfer", command=transfer_ticket, bootstyle="success")
